...
async function fetchCourses(group) {
    ...
    try {
        ...
        if (process.env.LOGS_RECUP_GPES == "true") {
            ...
            console.log(
                `Temps de traitement : ${parseFloat(
                    "" + processingTime / 1000,
                ).toFixed(2)}s | Temps de traitement moyen : ${parseFloat(
                    "" +
                        averageProcessingTime.timeSum /
                            averageProcessingTime.measuresNumber /
                            1000,
                ).toFixed(2)}s`,
            );
...
async function getCourses() {
    ...
    console.log(
        `${groupsNumber} groupes seront traités toutes les ${
            CYCLE_INTERVAL / 1000 / 60 / 60
        }h`,
    );
    console.log(
        `Intervalle entre chaque groupe : ${
            intervalBetweenGroups / 1000
        } secondes`,
    );
    startUpdateCycle();
}
