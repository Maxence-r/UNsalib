...
async function getStats() {
    ...
    statsPage.querySelector('.page-title h2').textContent = `${new Date().getMonth() + 1 > 9 ? new Date().getMonth() + 1 : '0' + (new Date().getMonth() + 1)}/${new Date().getFullYear()} (actualisé à ${new Date().getHours() > 9 ? new Date().getHours() : '0' + new Date().getHours()}:${new Date().getMinutes() > 9 ? new Date().getMinutes() : '0' + new Date().getMinutes()})`;
    try {
