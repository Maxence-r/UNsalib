...
function orderCoursesByDay(
    ...
    // End day included
    const nbDays = weekEndDate - weekStartDate + 1;
    ...
    courses.forEach((course) => {
        const courseDate = new Date(course.start).getDate();
        coursesByDay[courseDate - weekStartDate].push(course);
    });
...
function Grid({
    ...
    const orderedCourses: ApiCourse[][] = useMemo(
        () =>
            orderCoursesByDay(courses, weekStartDate, weekEndDate).filter(
                (_val, i) => i < VISIBLE_DAYS,
            ),
        [courses, weekStartDate, weekEndDate],
    );
